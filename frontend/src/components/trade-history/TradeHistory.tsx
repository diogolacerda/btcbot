...
import type { TradeHistoryProps, Trade, SortConfig } from './types'
...
export function TradeHistory({
  ...
  // Use totalTrades from props if available (server-side), otherwise calculate from trades array
  const total = totalTrades ?? trades.length
  const totalPages = Math.ceil(total / tradesPerPage)
  const startIndex = (currentPage - 1) * tradesPerPage + 1
  const endIndex = Math.min(currentPage * tradesPerPage, total)
        ...
        <div className="flex-1 space-y-4">
          ...
          <div className="text-sm text-slate-600 dark:text-slate-400">
            {isLoading ? (
              ...
              `Showing ${startIndex}-${endIndex} of ${total} trades`
            ) : (
              'No trades found'
            )}
          </div>
