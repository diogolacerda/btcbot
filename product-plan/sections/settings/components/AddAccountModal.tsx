...
interface AddAccountModalProps {
  isOpen: boolean
  onClose: () => void
  onAdd?: (account: Omit<Account, 'id' | 'createdAt' | 'lastTestedAt' | 'isActive' | 'connectionStatus'>) => void
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl max-w-md w-full">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          {/* Exchange (disabled for MVP - only BingX) */}
          <div>
            ...
            <input
              ...
              disabled
              className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-500 dark:text-slate-400 cursor-not-allowed"
            />
          </div>
            ...
            <div className="flex gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, mode: e.target.value as 'demo' })}
                  className="w-4 h-4 text-emerald-600 focus:ring-emerald-500"
                />
                <span className="text-sm text-slate-900 dark:text-slate-100">Demo</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="radio"
                  ...
                  onChange={(e) => setFormData({ ...formData, mode: e.target.value as 'live' })}
                  className="w-4 h-4 text-emerald-600 focus:ring-emerald-500"
                />
                <span className="text-sm text-slate-900 dark:text-slate-100">Live</span>
              </label>
            </div>
          </div>
