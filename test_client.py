    ...
    try:
        ...
        for pos in positions:
            if float(pos.get("positionAmt", 0)) != 0:
                print(
                    f"   - {pos.get('symbol')} {pos.get('positionSide')}: {pos.get('positionAmt')} @ {pos.get('avgPrice')}"
                )
...
async def test_get_open_orders(client: BingXClient, symbol: str):
    ...
    try:
        ...
        for order in orders[:5]:  # Mostrar até 5
            print(
                f"   - ID: {order.get('orderId')} | {order.get('side')} {order.get('positionSide')} | Preço: {order.get('price')} | Qtd: {order.get('origQty')}"
            )
        if not orders:
...
async def test_create_simple_limit_order(
    client: BingXClient, symbol: str, price: float, quantity: float
):
...
async def test_create_limit_order_with_tp(
    client: BingXClient, symbol: str, price: float, quantity: float
):
...
async def main():
    ...
    try:
        ...
        for attempt in range(3):
            order_result = await test_create_simple_limit_order(
                client, symbol, test_price, test_quantity
            )
            if order_result:
                break
        ...
        if order_result:
            order_id = str(
                order_result.get("orderId") or order_result.get("order", {}).get("orderId", "")
            )
            if order_id:
        ...
        await asyncio.sleep(1)
        order_result = await test_create_limit_order_with_tp(
            client, symbol, test_price, test_quantity
        )
