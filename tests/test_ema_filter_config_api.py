...
def mock_ema_filter_repo(sample_ema_config):
    ...
    mock = MagicMock()
    mock.get_by_strategy = AsyncMock(return_value=sample_ema_config)
    mock.create_or_update = AsyncMock(return_value=sample_ema_config)
    return mock
...
@pytest.fixture
def client(async_session, mock_strategy_repo, mock_ema_filter_repo, mock_user, account_id):
    ...
    async def override_get_ema_filter_repo(session=None):
...
class TestPatchEMAFilterConfig:
    ...
    def test_update_ema_filter_unauthorized(self, async_session):
